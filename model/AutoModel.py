...
from models.data_loader import AutoModelDataGenerator
...
def train(train_path, dev_path, test_path, pred_results_dir, n_labels, batch_size, epoch_num, lr, embedding_dim, k_id):
    # build a training model

    train_data_list = utils.json2list4brm(train_path)
    dev_data_list = utils.json2list4brm(dev_path)
    test_data_list = utils.json2list4brm(test_path)
    
    # BertConcate
    # train_data_list = utils.json2list4train_lf(train_path)
    # dev_data_list = utils.json2list4eval_lf(dev_path)
    # test_data_list = utils.json2list4eval_lf(test_path)

    # train_data_list = utils.json2list2train_sysu(train_path)
    # dev_data_list = utils.json2list2eval_sysu(dev_path)
    # test_data_list = utils.json2list2eval_sysu(test_path)
    ...
    model = AutoModelForSequenceClassification.from_pretrained(pretrained_name,
                                                                ...
                                                                hidden_dropout_prob=0.2).to(device)
    tokenizer = AutoTokenizer.from_pretrained(pretrained_name)
    
    train_data = AutoModelDataGenerator(train_data_list, batch_size, tokenizer, embedding_dim, device) 
    dev_data = AutoModelDataGenerator(dev_data_list, batch_size, tokenizer, embedding_dim, device)
    test_data = AutoModelDataGenerator(test_data_list, batch_size, tokenizer, embedding_dim, device)
    ...
    best_weighted_f1, best_precision, best_recall, best_qwk, best_adj_acc, best_mae = 0, 0, 0, 0, 0, 0
    ...
    for epoch_idx in range(epoch_num):
        ...
        test_loss, test_acc, test_adj_acc, \
        ...
        if dev_loss < best_dev_loss:
            ...
            # output_dir = './trained_models/exemplar_nov/roberta_large_' + str(batch_size) + '_' + str(lr) + '_' + str(epoch_num) + '/'
...
def AutoModel_K_fold(indir, n_labels, batch_size, epoch_num, lr, embedding_dim):
    # number fold
    fold_k = 10
    ...
    weighted_f1, precision, recall, qwk, adj_acc, mae = 0, 0, 0, 0, 0, 0

    # write best_epoch of each fold for further automated bash
    # best_epoch_dir = 'Sta_best_epochs_eng/'
    best_epoch_dir = 'Best_epochs/friend/' + model_name
    ...
    pred_results_dir_last = 'Pred_results/friend/' + model_name
    ...
    for k_id in range(fold_k):
        print(k_id)
        train_path = indir + '/' + str(k_id) + '/' + 'train.json'
        dev_path = indir + '/' + str(k_id) + '/' + 'dev.json'
        test_path = indir + '/' + str(k_id) + '/' + 'test.json'
        best_epoch_num, train_loss_temp, train_acc_temp, dev_loss_temp, dev_acc_temp, test_loss_temp, test_acc_temp, \
                test_f1_temp, test_precision_temp, test_recall_temp, test_qwk_temp, test_adj_acc_temp, test_mae_temp \
                 = train(train_path, dev_path, test_path, pred_results_dir, n_labels, batch_size, epoch_num, lr, embedding_dim, k_id)
